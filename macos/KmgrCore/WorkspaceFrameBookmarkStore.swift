...
/// frame. The rectangle itself lives in AppKit's named-frame preferences; this
/// small record gives a context a stable, opaque name for that rectangle.
public struct WorkspaceFrameBookmark: Hashable, Codable, Sendable, Identifiable {
    ...
    public static let frameAutosaveNamePrefix = "Kmgr-ClusterContextFrame-"
...
public final class WorkspaceFrameBookmarkStore {
    ...
    /// The bookmark identity is retained when the source changes, so the
    /// AppKit frame key remains stable as windows are activated in turn.
    @discardableResult
    ...
    /// Creates a context bookmark only when no history exists. Automatic
    /// relaunch restoration uses this to seed metadata for installations that
    /// predate frame bookmarks without replacing an established source.
    @discardableResult
    public func ensure(
        contextReference: String,
        sourceWindowID: String
    ) throws -> WorkspaceFrameBookmark {
        if let existing = bookmark(for: contextReference) {
            return existing
        }
        return try activate(
            contextReference: contextReference,
